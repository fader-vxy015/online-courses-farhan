...
// Total pendaftar
$total_pendaftar_query = mysqli_query($conn, "SELECT COUNT(*) as total FROM pendaftar");
$total_pendaftar = mysqli_fetch_assoc($total_pendaftar_query)['total'];

// Chart data: pendaftar per bulan
$month_counts = [];
$chart_monthly_query = mysqli_query($conn, "SELECT DATE_FORMAT(created_at, '%Y-%m') AS month, COUNT(*) AS total FROM pendaftar GROUP BY month ORDER BY month ASC");
while ($row = mysqli_fetch_assoc($chart_monthly_query)) {
...
// Chart data: distribusi jenis kursus
$course_counts = [];
$chart_course_query = mysqli_query($conn, "SELECT jenis_kursus, COUNT(*) AS total FROM pendaftar GROUP BY jenis_kursus ORDER BY total DESC");
while ($row = mysqli_fetch_assoc($chart_course_query)) {
    $course_counts[] = [
        'label' => $row['jenis_kursus'] ?: 'Lainnya',
        'value' => (int) $row['total'],
    ];
}

$course_labels = array_column($course_counts, 'label');
$course_values = array_column($course_counts, 'value');
?>
<!DOCTYPE html>
<html lang="id">
<head>
  ...
  <div class="layout">
    <aside class="sidebar">
      ...
      <ul>
        ...
        <li><a href="analytics.php" class="active"><i class="fas fa-chart-pie"></i> <span>Analitik</span></a></li>
